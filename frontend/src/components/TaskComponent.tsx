...
// const TaskComponent = ({ task, onStartEdit,onDelete
//  }: Props) => {
//   const [isHovered, setIsHovered] = useState(false);
//   const {users, response} = useAppSelector(state=>state.user)
//   const handleClickInfo = () => {
//     setIsHovered(prew=>!prew)
//   };
...
//     >
//       <Box>
//         <Typography variant="h4" color={response.userId===task.assignee? "":"gray"}>{task.title}</Typography>
//         {isHovered && (
//           <>
//            <Typography>Assignee: {users.find(user => user._id === task.assignee)?.name||""}</Typography>
//             <Typography>Status: {task.status}</Typography>
//             <Typography>Location: {task.location.name}</Typography>
//             <Typography>Task due date: {new Date(task.date).toLocaleString()}</Typography>
//             <Typography>Description: {task.description}</Typography>
//           </>
//         )}
//       </Box>
...
//           <InfoIcon sx={{color:isHovered?"#4caf50":"rgba(0, 0, 0, 0.54" }}/>
//         </IconButton>
